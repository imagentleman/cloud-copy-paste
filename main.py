...
INITIAL_OBJECT = {
    ...
    "exampleText": ("All devices with this page opened will be kept synced with the same data. "
    "We autogenerated a url, but you can choose anything (/your-secret-name)."
    "We'll detect and insert images and urls automatically for you.")
}
...
def _get_firebase_db_url():
    return "https://cloud-copy-paste.firebaseio.com"
...
def generate_uri():
    ...
    for i in xrange(3):
        uri += random.choice(string.ascii_lowercase)

    return uri

@app.route('/get/<clipboard>')
def get_clipboard(clipboard):
    url = '{}/{}.json'.format(_get_firebase_db_url(), clipboard)
    return flask.jsonify(firebase_get(url))

@app.route('/post/<clipboard>', methods=['POST'])
def post_clipboard(clipboard):
    url = '{}/{}.json'.format(_get_firebase_db_url(), clipboard)
...
def clipboard(clipboard):
    ...
    if data is None:
        firebase_put(url, json.dumps(INITIAL_OBJECT))
